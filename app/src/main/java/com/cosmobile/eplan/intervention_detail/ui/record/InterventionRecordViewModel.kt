package com.cosmobile.eplan.intervention_detail.ui.record
...
constructor(
    ...
    fun setStart(time: LocalTime) {
        ...
        savedStateHandle[STATE_KEY_START] = start.value
        isRecording.value = true
        savedStateHandle[STATE_KEY_ISRECORDING] = isRecording.value
        isOver.value = false
    }

    fun setEnd(time: LocalTime) {
        end.value = time.truncatedTo(ChronoUnit.MINUTES).toString()
        savedStateHandle[STATE_KEY_END] = end.value
